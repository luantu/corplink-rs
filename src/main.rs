...
use corplink_rs::{
    client::{self, Client},
    config::{read_check_config, Config, WgConf},
    utils::{self, get_interface_address, print_version, send_feishu_message},
    wg, yaml,
};
use flexi_logger::{
    DeferredNow, FileSpec, FlexiLoggerError, Logger, LoggerHandle, Record, WriteMode,
};
use log;
...
#[cfg(target_os = "macos")]
use corplink_rs::dns::{self, DNSManager};
                            ...
                            if let Some(expires_str) = expires.as_str() {
                                    ...
                                    chrono::DateTime::parse_from_rfc3339(expires_str)
                                {
                                    ...
                                    if time_left <= one_day && time_left > chrono::Duration::zero()
                                    {
                                        ...
                                        log::info!("{}", message);
                                        if let Err(err) =
                                            send_feishu_message(feishu_webhook_url, &message).await
                                        {
                                            log::warn!(
                                                "Failed to send cookie expiry notification: {}",
                                                err
                                            );
                                        }
                                    }
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut conf = match std::panic::catch_unwind(|| {
        ...
        async move { Config::from_file(&file).await }
    }) {
    ...
    let feishu_webhook_url = check_config
        .as_ref()
        .map(|c| c.feishu_webhook_url.clone())
        .unwrap_or_default();
    ...
    std::panic::set_hook(Box::new(move |panic_info| {
        ...
        if !feishu_webhook_url_clone_panic.is_empty() {
            if let Ok(rt) = tokio::runtime::Builder::new_current_thread().build() {
                let _ = rt.block_on(utils::send_feishu_message(
                    &feishu_webhook_url_clone_panic,
                    &message,
                ));
            }
        }
    ...
    loop {
        ...
        loop {
            ...
            if c.need_login() {
                log::info!(
                    "not login yet, try to login (attempt {})",
                    connection_attempts
                );
                match c.login().await {
                    ...
                    Err(e) => {
                        ...
                        retry_interval =
                            std::cmp::min(retry_interval * 2, MAX_RETRY_INTERVAL_SECONDS);
                        continue;
                    }
                };
            }
            ...
            match c.connect_vpn().await {
                ...
                Err(e) => {
                    if logout_retry && e.to_string().contains("logout") {
                        ...
                        log::info!(
                            "Waiting {} seconds before retrying... (attempt {})",
                            retry_interval,
                            connection_attempts
                        );

                        // 发送飞书通知
                        let feishu_url = check_config
                            .as_ref()
                            .map(|c| c.feishu_webhook_url.clone())
                            .unwrap_or_default();
        ...
        if wg_conf.use_intranet {
            ...
            if let Some(cc) = &check_config {
                if !cc.config_yaml_path.is_empty() {
                    if let Err(e) = yaml::update_config_yaml(
                        &cc.config_yaml_path,
                        intranet_ip,
                        &cc.proxy_name_to_update,
                        &cc.svn_username,
                        &cc.svn_password,
                    ) {
                        ...
                        log::info!(
                            "Successfully updated {} server address to {}",
                            cc.proxy_name_to_update,
                            intranet_ip
                        );
                    }
            ...
            let feishu_url = check_config
                ...
                .map(|c| c.feishu_webhook_url.clone())
                .unwrap_or_default();
            let config_yaml_path = check_config
                .as_ref()
                .map(|c| c.config_yaml_path.clone())
                .unwrap_or_default();
            let proxy_name = check_config
                .as_ref()
                .map(|c| c.proxy_name_to_update.clone())
                .unwrap_or_default();
            let svn_username = check_config
                .as_ref()
                .map(|c| c.svn_username.clone())
                .unwrap_or_default();
            let svn_password = check_config
                .as_ref()
                .map(|c| c.svn_password.clone())
                .unwrap_or_default();
